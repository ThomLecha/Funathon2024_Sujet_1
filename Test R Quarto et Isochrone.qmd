...
# on remarque un problème de type car les lat et lng sont en chr alors qu'elles devraient être en num


# On obtient les coordonnées de la gare
LilleData <- gareData %>% 
  filter(libelle == GARE_NAME)
coordsLilleFlandres <- c(LilleData$lat[1],LilleData$lng[1])
...
getStationCoordinates <- function(gare, verbose = TRUE){
  ...
  coordinates <- c(data$lat[1],data$lng[1])
...
getCityCoordinates("Toulouse-Matabiau")
```
...
-   Lieu de départ : Paris-St-Lazare (48.8766, 2.3251)
    ...
    "departure_searches": [
      {
        ...
        "departure_time": "2024-12-02T18:00:00.000Z",
        "properties": [
          "travel_time",
          "route"
        ],
...
On crée une fonction qui prend en paramètre les noms de 2 villes et qui renvoie le temps de transport en heure entre ces 2 villes. On prendra soin de gérer le cas où aucun itinéraire n'est trouvé en renvoyant 'Inf'
